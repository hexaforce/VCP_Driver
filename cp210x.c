...
static int cp210x_tiocmset_port(struct usb_serial_port *port, unsigned int, unsigned int);
...
static struct usb_serial_driver *const serial_drivers[] = {&cp210x_device, NULL};
 ...
 */
static int cp210x_read_reg_block(struct usb_serial_port *port, u8 req, void *buf, int bufsize) {
  ...
  result = usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0), req, REQTYPE_INTERFACE_TO_HOST, 0, port_priv->bInterfaceNumber, dmabuf, bufsize, USB_CTRL_SET_TIMEOUT);
  if (result == bufsize) {
 ...
 * Returns data into buf in native USB byte order.
 */
static int cp210x_read_vendor_block(struct usb_serial *serial, u8 type, u16 val, void *buf, int bufsize) {
  void *dmabuf;
  int result;
  ...
  result = usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0), CP210X_VENDOR_SPECIFIC, type, val, cp210x_interface_num(serial), dmabuf, bufsize, USB_CTRL_GET_TIMEOUT);
  if (result == bufsize) {
    memcpy(buf, dmabuf, bufsize);
    result = 0;
  } else {
    dev_err(&serial->interface->dev, "failed to get vendor val 0x%04x size %d: %d\n", val, bufsize, result);
    if (result >= 0)
      result = -EIO;
  }
...
static int cp210x_write_u16_reg(struct usb_serial_port *port, u8 req, u16 val) {
  ...
  result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), req, REQTYPE_HOST_TO_INTERFACE, val, port_priv->bInterfaceNumber, NULL, 0, USB_CTRL_SET_TIMEOUT);
  if (result < 0) {
...
static int cp210x_write_reg_block(struct usb_serial_port *port, u8 req, void *buf, int bufsize) {
  ...
  result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), req, REQTYPE_HOST_TO_INTERFACE, 0, port_priv->bInterfaceNumber, dmabuf, bufsize, USB_CTRL_SET_TIMEOUT);
 ...
 * Data in buf must be in native USB byte order.
 */
static int cp210x_write_vendor_block(struct usb_serial *serial, u8 type, u16 val, void *buf, int bufsize) {
  void *dmabuf;
  int result;
  ...
  result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), CP210X_VENDOR_SPECIFIC, type, val, cp210x_interface_num(serial), dmabuf, bufsize, USB_CTRL_SET_TIMEOUT);
  ...
  if (result == bufsize) {
    ...
    dev_err(&serial->interface->dev, "failed to set vendor val 0x%04x size %d: %d\n", val, bufsize, result);
    if (result >= 0)
      result = -EIO;
  }
...
static void cp210x_close(struct usb_serial_port *port) {
  ...
  if (port_priv->is_cp2102n_a01) {
    ...
    result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), 0x09, 0, 0, port_priv->bInterfaceNumber, NULL, 0, USB_CTRL_SET_TIMEOUT);

    if (result < 0) {
      dev_err(&port->dev, "failed usb_control_msg (1) request 0x09 status: %d\n", result);
    }

    result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), 0x09, 0, 1, port_priv->bInterfaceNumber, NULL, 0, USB_CTRL_SET_TIMEOUT);

    if (result < 0) {
      dev_err(&port->dev, "failed usb_control_msg (2) request 0x09 status: %d\n", result);
    }

    result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), 0x01, 2, 0, 0x82, NULL, 0, USB_CTRL_SET_TIMEOUT);

    if (result < 0) {
      dev_err(&port->dev, "failed usb_control_msg (1) request 0x01 status: %d\n", result);
    }

    result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), 0x01, 2, 0, 0x02, NULL, 0, USB_CTRL_SET_TIMEOUT);
 ...
 * Returns data into buf in native USB byte order.
 */
static int cp210x_read_vs_reg_block(struct usb_serial_port *port, u8 bmRequestType, u16 wValue, void *buf, int bufsize) {
  struct usb_serial *serial = port->serial;
  struct cp210x_port_private *port_priv = usb_get_serial_port_data(port);
  void *dmabuf;
  int result;
  ...
  result = usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0), CP210X_VENDOR_SPECIFIC, bmRequestType, wValue, port_priv->bInterfaceNumber, dmabuf, bufsize, USB_CTRL_SET_TIMEOUT);
  if (result == bufsize) {
    memcpy(buf, dmabuf, bufsize);
    result = 0;
  } else {
 ...
 * Data in buf must be in native USB byte order.
 */
static int cp210x_write_gpio_reg_block(struct usb_serial_port *port, u8 bmRequestType, void *buf, int bufsize) {
  struct usb_serial *serial = port->serial;
  struct cp210x_port_private *port_priv = usb_get_serial_port_data(port);
  void *dmabuf;
  int result;
...
static int cp210x_ioctl(struct tty_struct *tty, unsigned int cmd, unsigned long arg) {
  ...
  switch (cmd) {
  case IOCTL_GPIOGET:
    if ((port_priv->bPartNumber == CP210X_PARTNUM_CP2103) || (port_priv->bPartNumber == CP210X_PARTNUM_CP2104) || (port_priv->bPartNumber == CP210X_PARTNUM_CP2102N_QFN28) || (port_priv->bPartNumber == CP210X_PARTNUM_CP2102N_QFN24) || (port_priv->bPartNumber == CP210X_PARTNUM_CP2102N_QFN20)) {
      u8 gpio;
 ...
 */
static int cp210x_get_tx_queue_byte_count(struct usb_serial_port *port, u32 *count) {
  struct usb_serial *serial = port->serial;
  struct cp210x_port_private *port_priv = usb_get_serial_port_data(port);
  ...
  result = usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0), CP210X_GET_COMM_STATUS, REQTYPE_INTERFACE_TO_HOST, 0, port_priv->bInterfaceNumber, sts, sizeof(*sts), USB_CTRL_GET_TIMEOUT);
...
static void cp210x_set_termios(struct tty_struct *tty, struct usb_serial_port *port, const struct ktermios *old_termios) {
  ...
  if (iflag & IXON) {
     ...
     */
    if (priv->partnum == CP210X_PARTNUM_CP2102N_QFN20 || priv->partnum == CP210X_PARTNUM_CP2102N_QFN24 || priv->partnum == CP210X_PARTNUM_CP2102N_QFN28)
...
static int cp210x_suspend(struct usb_serial *serial, pm_message_t message) {
  ...
  for (i = 0; i < serial->num_ports; i++) {
    if (serial->port[i]->port.tty != NULL) {
      cp210x_read_reg_block(serial->port[i], CP210X_GET_FLOW, &flow_ctl, sizeof(flow_ctl));
      ctl_hs = le32_to_cpu(flow_ctl.ulControlHandshake);
      flow_repl = le32_to_cpu(flow_ctl.ulFlowReplace);
      ctl_hs &= ~CP210X_SERIAL_CTS_HANDSHAKE;
      flow_repl &= ~CP210X_SERIAL_RTS_MASK;
      ...
      flow_ctl.ulControlHandshake = cpu_to_le32(ctl_hs);
      flow_ctl.ulFlowReplace = cpu_to_le32(flow_repl);
      result = cp210x_write_reg_block(serial->port[i], CP210X_SET_FLOW, &flow_ctl, sizeof(flow_ctl));
      if (result < 0) {
...
static int cp210x_tiocmset_port(struct usb_serial_port *port, unsigned int set, unsigned int clear) {
  u16 control = 0;
  struct cp210x_flow_ctl flow_ctl;
  u32 ctl_hs = 0;
  u32 flow_repl = 0;
...
static int cp210x_break_ctl(struct tty_struct *tty, int break_state) {
  ...
  if (break_state == 0)
    ...
    state = BREAK_ON;
  dev_dbg(&port->dev, "%s - turning break %s\n", __func__, state == BREAK_OFF ? "off" : "on");
  int result = cp210x_write_u16_reg(port, CP210X_SET_BREAK, state);
  return result;
}
...
static int cp210x_gpio_get(struct gpio_chip *gc, unsigned int gpio) {
  ...
  if (priv->partnum == CP210X_PARTNUM_CP2108) {
    ...
    if (result)
      return result;
    result = cp210x_read_vendor_block(serial, req_type, CP210X_READ_LATCH, &buf, sizeof(buf));
...
static void cp210x_gpio_set(struct gpio_chip *gc, unsigned int gpio, int value) {
  ...
  if (priv->partnum == CP210X_PARTNUM_CP2105)
    result = cp210x_write_vendor_block(serial, REQTYPE_HOST_TO_INTERFACE, CP210X_WRITE_LATCH, &u8_buf, sizeof(u8_buf));
    ...
    result = cp210x_write_vendor_block(serial, REQTYPE_HOST_TO_INTERFACE, CP210X_WRITE_LATCH, &u16_buf, sizeof(u16_buf));
  } else {
    u16 wIndex = u8_buf.state << 8 | u8_buf.mask;
    result = usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0), CP210X_VENDOR_SPECIFIC, REQTYPE_HOST_TO_DEVICE, CP210X_WRITE_LATCH, wIndex, NULL, 0, USB_CTRL_SET_TIMEOUT);
  }
  usb_autopm_put_interface(serial->interface);
out:
  if (result < 0) {
...
static int cp210x_gpio_direction_output(struct gpio_chip *gc, unsigned int gpio, int value) {
  struct usb_serial *serial = gpiochip_get_data(gc);
  struct cp210x_serial_private *priv = usb_get_serial_data(serial);
...
static int cp210x_gpio_set_config(struct gpio_chip *gc, unsigned int gpio, unsigned long config) {
  struct usb_serial *serial = gpiochip_get_data(gc);
  struct cp210x_serial_private *priv = usb_get_serial_data(serial);
  enum pin_config_param param = pinconf_to_config_param(config);

  /* Succeed only if in correct mode (this can't be set at runtime) */
  if ((param == PIN_CONFIG_DRIVE_PUSH_PULL) && (priv->gpio_pushpull & BIT(gpio)))
    return 0;

  if ((param == PIN_CONFIG_DRIVE_OPEN_DRAIN) && !(priv->gpio_pushpull & BIT(gpio)))
    return 0;
...
static int cp2105_gpioconf_init(struct usb_serial *serial) {
  ...
  result = cp210x_read_vendor_block(serial, REQTYPE_DEVICE_TO_HOST, CP210X_GET_DEVICEMODE, &mode, sizeof(mode));
  if (result < 0)
    return result;

  result = cp210x_read_vendor_block(serial, REQTYPE_DEVICE_TO_HOST, CP210X_GET_PORTCONFIG, &config, sizeof(config));
  if (result < 0)
    return result;
  ...
  if (intf_num == 0) {
    ...
    priv->gpio_pushpull = (u8)((le16_to_cpu(config.gpio_mode) & CP210X_ECI_GPIO_MODE_MASK) >> CP210X_ECI_GPIO_MODE_OFFSET);
    priv->gc.ngpio = 2;
  } else if (intf_num == 1) {
    ...
    priv->gpio_pushpull = (u8)((le16_to_cpu(config.gpio_mode) & CP210X_SCI_GPIO_MODE_MASK) >> CP210X_SCI_GPIO_MODE_OFFSET);
    priv->gc.ngpio = 3;
  } else {
    return -ENODEV;
  }
...
static int cp2104_gpioconf_init(struct usb_serial *serial) {
  ...
  priv->gpio_pushpull = (u8)((le16_to_cpu(config.gpio_mode) & CP210X_GPIO_MODE_MASK) >> CP210X_GPIO_MODE_OFFSET);
  gpio_latch = (u8)((le16_to_cpu(config.reset_state) & CP210X_GPIO_MODE_MASK) >> CP210X_GPIO_MODE_OFFSET);
...
static int cp2108_gpio_init(struct usb_serial *serial) {
  ...
  result = cp210x_read_vendor_block(serial, REQTYPE_DEVICE_TO_HOST, CP210X_GET_PORTCONFIG, &config, sizeof(config));
  if (result < 0) {
    dev_warn(&serial->interface->dev, "querying port config on cp2108 failed\n");
    return result;
  }
  priv->gc.ngpio = 16;
  iface_config = config.EnhancedFxn_IFC[0];
  priv->gpio_pushpull = (u16)((le16_to_cpu(config.reset_state.Mode_PB[1]) & CP2108_GPIO_MODE_MASK) >> CP2108_GPIO_MODE_OFFSET);
  gpio_latch = (u16)((le16_to_cpu(config.reset_state.Latch_PB[1]) & CP2108_GPIO_MODE_MASK) >> CP2108_GPIO_MODE_OFFSET);
...
static int cp2102n_gpioconf_init(struct usb_serial *serial) {
  ...
  result = cp210x_read_vendor_block(serial, REQTYPE_DEVICE_TO_HOST, CP210X_READ_2NCONFIG, config_buf, config_size);
  if (result < 0) {
    kfree(config_buf);
    return result;
  }
...
static int cp210x_port_probe(struct usb_serial_port *port) {
  ...
  ret = cp210x_read_vs_reg_block(port, REQTYPE_DEVICE_TO_HOST, CP210X_GET_PARTNUM, &port_priv->bPartNumber, 1);
  if (ret) {
    kfree(port_priv);
    return ret;
  }

  if ((port_priv->bPartNumber == CP210X_PARTNUM_CP2102N_QFN28) || (port_priv->bPartNumber == CP210X_PARTNUM_CP2102N_QFN24) || (port_priv->bPartNumber == CP210X_PARTNUM_CP2102N_QFN20)) {
...
static int cp210x_attach(struct usb_serial *serial) {
  ...
  result = cp210x_read_vendor_block(serial, REQTYPE_DEVICE_TO_HOST, CP210X_GET_PARTNUM, &priv->partnum, sizeof(priv->partnum));
  if (result < 0) {
    dev_warn(&serial->interface->dev, "querying part number failed\n");
